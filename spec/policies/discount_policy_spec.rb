...
# spec/policies/discount_policy_spec.rb

require "spec_helper"

RSpec.describe DiscountPolicy, type: :policy do
  let!(:discount) { create(:discount, :with_country) }

  context "when admin is logged in" do
    let(:admin) { build(:admin, :confirmed) }
    subject { described_class.new(admin, discount) }
    ...
    it { is_expected.to match_policy_scope(admin, [discount]) }
  end

  context "when manager is logged in" do
    let(:manager) { build(:manager, :confirmed) }
    subject { described_class.new(manager, discount) }
  ...
  context "when cashier is logged in" do
    ...
    subject { described_class.new(cashier, discount) }
