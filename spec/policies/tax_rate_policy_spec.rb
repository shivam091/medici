...
# spec/policies/tax_rate_policy_spec.rb

require "spec_helper"

RSpec.describe TaxRatePolicy, type: :policy do
  let!(:tax_rate) { create(:tax_rate, :with_country) }

  context "when admin is logged in" do
    let(:admin) { build(:admin, :confirmed) }
    subject { described_class.new(admin, tax_rate) }
    ...
    it { is_expected.to match_policy_scope(admin, [tax_rate]) }
  end

  context "when manager is logged in" do
    let(:manager) { build(:manager, :confirmed) }
    subject { described_class.new(manager, tax_rate) }
  ...
  context "when cashier is logged in" do
    ...
    subject { described_class.new(cashier, tax_rate) }
