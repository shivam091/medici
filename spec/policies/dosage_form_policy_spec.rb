...
# spec/policies/dosage_form_policy_spec.rb
...
RSpec.describe DosageFormPolicy, type: :policy do
  ...
  context "when admin is logged in" do
    ...
    it { is_expected.to match_policy_scope(admin, [dosage_form]) }
  end

  context "when manager is logged in" do
    let(:manager) { build(:manager, :confirmed) }
    subject { described_class.new(manager, dosage_form) }
  ...
  context "when cashier is logged in" do
    ...
    subject { described_class.new(cashier, dosage_form) }
