...
module PurchaseOrdersHelper
  def purchase_orders_path
    case
    when current_user.super_admin? then admin_purchase_orders_path
  ...
  def pending_purchase_orders_path
    case
    when current_user.super_admin? then pending_admin_purchase_orders_path
  ...
  def incomplete_purchase_orders_path
    case
    when current_user.super_admin? then incomplete_admin_purchase_orders_path
  ...
  def new_purchase_order_path
    case
    when current_user.super_admin? then new_admin_purchase_order_path
  ...
  def edit_purchase_order_path(purchase_order)
    case
    when current_user.super_admin? then edit_admin_purchase_order_path(purchase_order)
  ...
  def purchase_order_path(purchase_order)
    case
    when current_user.super_admin? then admin_purchase_order_path(purchase_order)
  ...
  def purchase_order_object(purchase_order)
    case
    when current_user.super_admin? then [:admin, purchase_order]
